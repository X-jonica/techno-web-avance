    ...
    export default {
        ...
        async created() {
            try {
                ...
                const productResponse = await axios.get(`http://localhost:4000/api/chossures/${id}`)
                this.product = productResponse.data.data

                // Charger tous les produits pour la navigation
                const allProductsResponse = await axios.get('http://localhost:4000/api/chossures')
            ...
            async loadProductData(productId) {
                try {
                    const response = await axios.get(
                        `http://localhost:4000/api/chossures/${productId}`
                    )
